        ...
        <table className='text-[#1E2435]'>
          <thead>
            <tr className='bg-[#32D096]'>
              ...
              {hours.map((hour, idx) => {
                return (
                  <th key={idx} className='p-2 text-left border-2 border-green-600'>{hour}</th>
                );
              })}
              ...
              return (
                <tr key={idx} className='text-right'>
                   {/*https://chat.openai.com/c/1d072a3b-6bdf-478b-a5f0-9038dc5b8265*/}
                    <td className={idx % 2 === 0 
                    ? 'p-1 border-2 border-green-600 bg-[#A8F4D0] font-bold' 
                    : 'p-1 border-2 border-green-600 font-bold bg-[#6FE6B7]'}>
                      {report.location}
                      <button className='text-[#E79088] text-justify' onClick={deleteCookieStand}>{trash_icon}</button>
                    </td>
                  {report.hourly.map((hour, index) => {
                    return (
                          <td key={index} className={idx % 2 === 0 
                            ? 'border-green-600 border-2 bg-[#A8F4D0] px-1' 
                            : 'border-green-600 border-2 bg-[#6FE6B7] px-1'}>{hour}
                          </td>
                    )
                  })}
                  <td className={idx % 2 === 0 ?
